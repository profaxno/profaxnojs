...
export class PfxHttpService {
  private readonly logger = new Logger(PfxHttpService.name);
  ...
  async post<T>(path: string, body: Record<string, any>, params?: Record<string, any>): Promise<T> {
    this.logger.log(`post: path=${path}, body=${this.cleanBodyForLog(body)}, params=${JSON.stringify(params)}`);
  ...
  async put<T>(path: string, body: Record<string, any>, params?: Record<string, any>): Promise<T> {
    this.logger.log(`put: path=${path}, body=${this.cleanBodyForLog(body)}, params=${JSON.stringify(params)}`);
  ...
  async request<T>(httpMethod: PfxHttpMethodEnum, url: string, headers: any, body?: Record<string, any>, params?: Record<string, any>): Promise<T> {
    this.logger.log(`request: httpMethod=${httpMethod}, url=${url}, body=${this.cleanBodyForLog(body)}, params=${JSON.stringify(params)}`);
